...
int	t_map_polynomial_plane_validate_repeat_coefficient_digit_repeat(char c)
{
	if ('0' <= c && c <= '9')
		return (
			T_MAP_POLYNOMIAL_PLANE_PARSE_STATE_REPEAT_COEFFICIENT_DIGIT_REPEAT);
	if (c == '.')
		return (
			T_MAP_POLYNOMIAL_PLANE_PARSE_STATE_REPEAT_COEFFICIENT_DOT);
	if (c == 'x')
		return (
