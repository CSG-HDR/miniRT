...
static bool is_in_square(t_map_cube cube, t_map_position point)
{
	...
	if (point.z < cube.position.z ||
		...
		return (false);
	if (point.x < cube.position.x + cube.size.x - SMALL_NUM ||
		point.x > cube.position.x + cube.size.x + SMALL_NUM)
		return (false);
	return (true);
}
			...
			t_ray_hit_records_builder *builder)
{
	...
	l.distance = (-ray.origin.x + cube.size.x) / ray.direction.x;
	...
	return (t_ray_hit_records_builder_add(builder, (t_ray_hit_record){
		l.distance,
		(t_map_normal){1, 0, 0},
		t_ray_material_from_color(cube.material_right),
		l.is_front_face,
		l.x,
		l.y
		})
