...
static bool is_in_square(t_map_cube cube, t_map_position point)
{
	...
	if (point.y < cube.position.y || point.y > cube.position.y + cube.size.y)
		return (false);
	if (point.z < cube.position.z + cube.size.z - SMALL_NUM ||
		point.z > cube.position.z + cube.size.z + SMALL_NUM)
		return (false);
	return (true);
}
			...
			t_ray_hit_records_builder *builder)
{
	...
	l.distance = t_f_abs(ray.origin.z * 1 - cube.size.z);
	l.point = t_f3_add(ray.origin, t_f3_mul((t_map_normal){0,0,1}, l.distance));
